...
class NetonHttpClientProxyTest {
    ...
    fun createAcceptsHttpProxyUrl() {
        ...
        val client = NetonHttpClient.create { proxyUrl = "http://127.0.0.1:1" }
        assertTrue(client is NetonHttpClient)
    }

    @Test
    fun malformedProxyUrlFailsFast() {
        assertFailsWith<NetonHttpException> {
            NetonHttpClient.create { proxyUrl = "not-a-url" }
        }
    }
    ...
    fun socksProxyRejected() {
        ...
        assertFailsWith<NetonHttpException> {
            NetonHttpClient.create { proxyUrl = "socks5://127.0.0.1:1080" }
        }
    }
}
