...
internal class SqlxEntityQuery<T : Any>(
    ...
    private val deletedColumn: ColumnRef?
) : EntityQuery<T> {

    override suspend fun list(): List<T> {
        val normalized = ast.normalizeForSoftDelete(deletedColumn)
    ...
    override fun select(vararg columnNames: String): ProjectionQuery {
        ...
        return SqlxProjectionQuery(adapter, newAst, deletedColumn)
    }
}
...
internal suspend fun <T : Any, ID : Any> SqlxTableAdapter<T, ID>.executePhase1Select(built: BuiltSql, rowMapper: RowMapper<T>): List<T> {
