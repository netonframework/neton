...
object RoutingComponent : NetonComponent<RequestEngine> {
    ...
    override suspend fun init(ctx: NetonContext, config: RequestEngine) {
        ...
        val rawConfig = ConfigLoader.loadModuleConfig("routing", configPath = "config", environment = ConfigLoader.resolveEnvironment(ctx.args), args = ctx.args)
        val routingConfig = parseRoutingConfig(rawConfig)
        if (routingConfig != null) {
            val configuredGroups = neton.core.interfaces.ConfiguredRouteGroups(routingConfig.groups.map { it.name }.toSet())
    ...
    private fun parseRoutingConfig(rawConfig: Map<String, Any?>?): RoutingConfig? {
        ...
        try {
            ...
            val routeGroups = groupsData.mapNotNull { item ->
                ...
                RouteGroup(name, RouteMountConfig(RouteMountType.PATH, mount))
            }
