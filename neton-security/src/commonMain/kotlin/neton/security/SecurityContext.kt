...
object SecurityContext : AuthenticationContext {
     ...
     * 返回 Any? 类型以适配不同的认证主体类型
     */
    override fun currentUser(): Any? = currentPrincipal
    
    /**
     * 获取当前用户（Principal类型）
     * @return 当前用户主体，未认证时返回 null
     */
    fun currentPrincipal(): Principal? = currentPrincipal
    
    /**
     * 检查用户是否已认证
     * @return true 如果用户已认证
     */
    fun isAuthenticated(): Boolean = currentPrincipal() != null
    ...
    fun hasRole(role: String): Boolean = 
        currentPrincipal()?.hasRole(role) ?: false
    ...
    fun hasAnyRole(vararg roles: String): Boolean = 
        currentPrincipal()?.hasAnyRole(*roles) ?: false
    ...
    fun hasAllRoles(vararg roles: String): Boolean = 
        currentPrincipal()?.hasAllRoles(*roles) ?: false
    ...
    fun hasPermission(permission: String): Boolean {
        val user = currentPrincipal() ?: return false
     ...
     */
    fun getAttribute(key: String): Any? = currentPrincipal()?.attributes?.get(key)
    
    /**
     * 获取用户ID
     * @return 用户ID，未认证时返回 null
     */
    fun userId(): String? = currentPrincipal()?.id
    
    /**
     * 获取用户角色列表
     * @return 用户角色列表，未认证时返回空列表
     */
    fun userRoles(): List<String> = currentPrincipal()?.roles ?: listOf()
    ...
    fun requireAuthenticated(): Principal {
        return currentPrincipal() ?: throw IllegalStateException("Authentication required")
    }
