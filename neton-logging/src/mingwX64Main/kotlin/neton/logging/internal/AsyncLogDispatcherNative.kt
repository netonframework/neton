...
private fun runWriterLoop(
    ...
    val intervalMs = flushIntervalMs.toUInt()
...
/**
 * Windows 异步分发器：单 writer 线程 + 有界队列。
 * 使用 Windows Sleep() 替代 POSIX usleep()。
 */
internal class AsyncLogDispatcherNative(
    ...
    init {
        ...
        worker.execute(
            TransferMode.SAFE,
            { listOf(sinks, queue, running, flushIntervalMs, maxBatch) }) { args ->
    ...
    override fun dispatch(sinkKeys: Set<String>, line: String, level: LogLevel) {
        ...
        if (queue.load().size >= queueCapacity) {
            if (neverDrop) {
                ...
                while (queue.load().size >= queueCapacity && running.load() != 0) Sleep(1u)
            } else {
    ...
    private fun maybeWarnDropped() {
        ...
        if (now - lastDroppedWarnTime.load() >= droppedWarnIntervalSec) {
            ...
            if (d > 0) {
                ...
                val warnLine =
                    """{"ts":"$ts","level":"WARN","msg":"log.dropped","dropped":$d,"queueSize":$queueCapacity,"flushBatchSize":$maxBatch,"flushEveryMs":$flushIntervalMs}""" + "\n"
    ...
    override fun close() {
        ...
        if (remaining > 0) {
            val ts = kotlin.time.Clock.System.now().toString()
            val errLine =
                """{"ts":"$ts","level":"ERROR","msg":"log.flush_timeout","remaining":$remaining,"timeoutMs":$shutdownFlushTimeoutMs}""" + "\n"
